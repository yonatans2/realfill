...
from torchvision import models as models_vgg
...
class NetVGGFeatures(torch.nn.Module):
    ...
    def __init__(self, layer_ids):
        ...
        self.vggnet = models_vgg.vgg16(pretrained=True)
...
class VGGDistance(torch.nn.Module):
    ...
    def forward(self, I1, I2):
        ...
        loss = torch.abs(I1 - I2).reshape(b_sz, -1).mean(1)

        for i in range(len(self.layer_ids)):
            layer_loss = torch.abs(f1[i] - f2[i]).reshape(b_sz, -1).mean(1)
...
class CLIPLoss(torch.nn.Module):
    ...
    def forward(self, model_preds, targets, mode="train"):
        if True:

            targets_ = self.preprocess(transforms.functional.to_pil_image(targets)).unsqueeze(0).to(self.device)
            # targets_ = targets_.permute(0,2,3,1)
        ...
        model_preds_ = self.preprocess(transforms.functional.to_pil_image(model_preds)).unsqueeze(0).to(self.device)
        pred_features = self.model.encode_image(model_preds_)
        loss_clip = 1. - torch.cosine_similarity(pred_features,self.targets_features,dim=1)
        # for n in range(self.NUM_AUGS):
...
def log_validation(
    ...
    if accelerator.trackers[0].name == "wandb":
        accelerator.trackers[0].log({f"validation": [wandb.Image(image, caption=str(i)) for i, image in enumerate(images)]})
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--vgg_weight", type=float, default=0.5, help="vgg loss weight")
    parser.add_argument("--clip_weight", type=float, default=0.5, help="clip loss weight")
...
def main(args):
    ...
    criterion_vgg = VGGDistance().to(accelerator.device)
    criterion_CLIP = CLIPLoss(device=accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, text_encoder):
                ...
                # tmp_clip = criterion_CLIP(model_pred[:,:3,:,:].float(),noise[:,:3,:,:].float())
                loss_clip = []
                for b in range(model_pred.shape[0]):
                     loss_clip.append(criterion_CLIP(model_pred[b,:3,:,:].float(),noise[b,:3,:,:].float()))
                temp_clip = torch.stack(loss_clip).mean()
                temp = 0.1*criterion_vgg(model_pred[:,:3,:,:].float(),noise[:,:3,:,:].float())
                loss = (weightings * F.mse_loss(model_pred.float(), noise.float(), reduction="none")).mean() + args.vgg_weight*temp + args.clip_weight*temp_clip
    ...
    if accelerator.is_main_process:
        ...
        inference_im_path = os.path.join(args.output_dir,"inferred_images")
        if not os.path.isdir(inference_im_path):
            os.mkdir(inference_im_path)
        for image_ind,image in enumerate(images):
            image.save(os.path.join(inference_im_path,"out_" + str(image_ind) + ".png" ))
